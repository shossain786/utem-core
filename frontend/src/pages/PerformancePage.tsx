    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
          <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">Avg Duration by Node Type</h2>
          {isLoading ? (
            <div className="h-48 flex items-center justify-center text-sm text-gray-400 dark:text-gray-500">Loading...</div>
          ) : chartData.length === 0 ? (
            <div className="h-48 flex items-center justify-center text-sm text-gray-400 dark:text-gray-500">No data yet</div>
          ) : (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
          <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Top 5 Slowest</h2>
          {isLoading ? (
            <div className="h-48 flex items-center justify-center text-sm text-gray-400 dark:text-gray-500">Loading...</div>
            ...
            <div className="space-y-2">
              {report.slowestTests.slice(0, 5).map((t, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-700 dark:text-gray-300 truncate flex-1" title={t.testName}>{t.testName}</span>
                  <span className="text-xs font-medium text-gray-900 dark:text-gray-100 shrink-0">{formatDuration(t.avgDurationMs)}</span>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
        <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-800">
          <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300">
            Slowest Tests
            {report && <span className="ml-2 text-xs font-normal text-gray-400 dark:text-gray-500">{report.slowestTests.length} tests</span>}
          </h2>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="p-8 text-center text-sm text-gray-400 dark:text-gray-500">No performance data yet. Run some tests first.</div>
        ) : (
                ...
                {report.slowestTests.map((t, i) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-gray-50 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                      <td className="px-4 py-2.5 text-xs text-gray-400 dark:text-gray-500">{i + 1}</td>
                      <td className="px-4 py-2.5 max-w-xs">
                        <span className="text-gray-900 dark:text-gray-100 truncate block" title={t.testName}>{t.testName}</span>
                      </td>
                      ...
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-medium text-gray-900 dark:text-gray-100">{formatDuration(t.avgDurationMs)}</span>
                        </div>
                      </td>
                      <td className="px-4 py-2.5 text-xs text-gray-500 dark:text-gray-400">{formatDuration(t.maxDurationMs)}</td>
                      <td className="px-4 py-2.5 text-xs text-gray-500 dark:text-gray-400">{formatDuration(t.minDurationMs)}</td>
                      <td className="px-4 py-2.5 text-xs text-gray-500 dark:text-gray-400">{t.runCount}</td>
                    </tr>
                  );
                })}
              </tbody>
...
function StatCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
      ...
      {sub && <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 truncate" title={sub}>{sub}</p>}
    </div>
  );
}
